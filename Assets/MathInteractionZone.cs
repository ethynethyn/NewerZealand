...
public class MathInteractionZone : MonoBehaviour
{
    ...
    {
        public string question;   // e.g. "12 + 8"
        public float answer;      // e.g. 20
    }
    ...
    void ShowQuestion()
    {
        ...
        promptText.text = questions[currentIndex].question;
    ...
    void CheckAnswer()
    {
        ...
        float correct = questions[currentIndex].answer;

        if (float.TryParse(inputField.text, out float playerAnswer))
        {
            if (Mathf.Approximately(playerAnswer, correct))
            {
                currentIndex++;
