...
public class BackpackController : MonoBehaviour
{
    ...
    [Tooltip("Seconds to wait before re-enabling colliders on retrieved items")]
    ...
    void HandleUseItem()
    {
        ...
        LootAndVendor lav = currentBackpack?.GetComponent<LootAndVendor>();

        GameObject item;

        if (lav != null)
            item = lav.TakeItem(selectedIndex);
        else
            item = currentStorage.RemoveItem(selectedIndex);
        ...
        // Re-enable colliders after a short delay so the bag
        // isn't pushed away as the item spawns out of it
        StartCoroutine(EnableCollidersDelayed(item, colliderEnableDelay));
    ...
    IEnumerator EnableCollidersDelayed(GameObject item, float delay)
    {
